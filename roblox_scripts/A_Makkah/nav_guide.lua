--[[ nav_guide.lua — LocalScript (taruh di StarterPlayer > StarterPlayerScripts)
  ...
  mengikuti jalur rute (ModuleScript "MakkahRoute" di ReplicatedStorage).
...
-- Ambil jalur navigasi: segmen ritual Mina (utamakan yang ada "Tawaf").
local routeMod = RS:FindFirstChild("MakkahRoute")
if not routeMod then warn("[Nav] 'MakkahRoute' tak ada."); return end
local data = HttpService:JSONDecode(require(routeMod))
local seg
for _, s in ipairs(data.segments or {}) do
	if string.find(s.ritual or "", "Tawaf") then seg = s; break end
end
