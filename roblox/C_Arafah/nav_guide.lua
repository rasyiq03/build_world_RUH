--[[ nav_guide.lua — LocalScript (taruh di StarterPlayer > StarterPlayerScripts)
  ...
  mengikuti jalur rute (ModuleScript "ArafahRoute" di ReplicatedStorage).
...
-- Ambil jalur navigasi: segmen ritual Mina (utamakan yang ada "wukuf").
local routeMod = RS:FindFirstChild("ArafahRoute")
if not routeMod then warn("[Nav] 'ArafahRoute' tak ada."); return end
local data = HttpService:JSONDecode(require(routeMod))
local seg
for _, s in ipairs(data.segments or {}) do
	if string.find(s.ritual or "", "wukuf") then seg = s; break end
end
