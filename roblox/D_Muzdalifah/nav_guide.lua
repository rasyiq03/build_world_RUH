--[[ nav_guide.lua — LocalScript (taruh di StarterPlayer > StarterPlayerScripts)
  ...
  mengikuti jalur rute (ModuleScript "MuzdalifahRoute" di ReplicatedStorage).
  WalkSpeed realistis; tahan SHIFT = PERCEPAT (ETA ikut memendek otomatis).

  Di Muzdalifah pemain mabit + ambil kerikil, lalu BERANGKAT ke Mina untuk
  lempar Jumrah Aqabah — jadi panduan default mengarah ke segmen KELUAR Muzdalifah.
...
-- Ambil jalur navigasi: utamakan segmen yang BERANGKAT dari Muzdalifah (menuju Mina/Jumrah),
-- lalu yang ritualnya "mabit", lalu segmen pertama.
local routeMod = RS:FindFirstChild("MuzdalifahRoute")
if not routeMod then warn("[Nav] 'MuzdalifahRoute' tak ada."); return end
local data = HttpService:JSONDecode(require(routeMod))
local seg
for _, s in ipairs(data.segments or {}) do
	if s.from == "Muzdalifah" then seg = s; break end
end
if not seg then
	for _, s in ipairs(data.segments or {}) do
		if string.find(string.lower(s.ritual or ""), "mabit") then seg = s; break end
	end
end
